...
module BodyParser
  ...
  BODY_HASH      = 'body.hash'.freeze

  def self.call(env, opts = {})
    if env[BODY_HASH].nil?
      env[BODY_HASH] = if env[RACK_INPUT].nil?
        {}
      else 
        opts[:async] ? parse_async(env, opts) : parse(env, opts)
      end
    end
  end

  def self.parse(env, opts = {})
    parse_stream(env[RACK_INPUT], env[TYPE_HEADER], opts)
  end

  def self.parse_async(env, opts = {})
  ...
  def self.parse_stream(stream, content_type, opts = {})
    max_size = opts[:max_size] || MAX_SIZE

    case content_type
    when JSON_TYPE
      JSONParser.parse(stream.read(max_size))
    when FORM_DATA_TYPE
  ...
  class Middleware
    ...
    def call(env)
      # initialize parser
      BodyParser.call(env, @opts)

      # call next handler
      status, headers, body = @app.call(env)

      # and remove files if any were created
      if !@opts[:skip_delete] && env[BODY_HASH].keys.any?
        files = env[BODY_HASH].select { |key, obj| obj.respond_to?(:file) && obj.file }
